...
export const usersRepository = {
  ...
  async resetPasswordWithATemporaryCode(
    id: string,
  ...
  async checkRecentPasswordResetById(id: string) {
    ...
    if (
      !user.accountData.recentPasswordReset ||
      !Array.isArray(user.accountData.recentPasswordReset)
    )
      return true;
  ...
  async findRecentUserConfirmationEmailsById(
    id: string,
  ...
  async addRecentLoginAttemptByEmail(email: string, attemptDate: Date) {
    const result = await usersCollection.updateOne(
      ...
      {
        $push: {
          "accountData.recentLoginAttempts": { attemptDate: attemptDate },
        },
      },
    );
    return !!result;
