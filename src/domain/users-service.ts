...
export const usersService = {
  async createUser(
    ...
    const hasRecentRegistration =
      await this.checkRecentRegistration(userIpAddress);
    if (hasRecentRegistration) return false;
    const possibleUser: UserDBModel | null =
    ...
    const sentEmail: boolean =
      await emailService.createAndSendEmailConfirmation(
        email,
        userName,
        newUser.id,
  ...
  async checkCredentials(
    ...
    if (!user.emailConfirmation.isConfirmed) {
      const recentConfirmationEmails =
        await usersRepository.findRecentUserConfirmationEmailsById(user.id);
      if (recentConfirmationEmails) {
        return {
          success: false,
          error:
            "Confirmation email has already been sent." +
            " Check your spam box, or try again later",
          statusCode: 429,
        };
      }
  ...
  async resetPassword(email: string) {
    ...
    const recentConfirmationEmails: UserDBModel | null =
      await usersRepository.findRecentUserConfirmationEmailsById(user.id);
    if (recentConfirmationEmails) {
      return false;
    }
    ...
    await usersRepository.resetPasswordWithATemporaryCode(
      user.id,
      passwordResetCode,
    );
  ...
  async resetPasswordWithToken(
    token: string,
    newPassword: string,
  ): Promise<AuthResetPasswordResponseModel> {
    ...
    const user: UserDBModel | null = await usersRepository.findUserById(
      tokenData.id,
    );
    ...
    const noRecentResets = await usersRepository.checkRecentPasswordResetById(
      user.id,
    );
    if (!noRecentResets)
      return {
        success: false,
        error: "Too many password resets in recent time",
      };
    ...
    await usersRepository.clearAllSessions(user.id);
  ...
  async checkRecentRegistration(ip: string): Promise<boolean> {
    const recentRegistration: UserDBModel[] =
      await usersRepository.findRecentUsersByIp(ip);
  ...
  async createAuthTokens(email: string, headersDeviceId?: string) {
    ...
    if (headersDeviceId) {
      await usersRepository.removeOutdatedRefreshToken(
        user.id,
        headersDeviceId,
      );
    }

    const updateResult = await usersRepository.updateRefreshTokenMeta(
      refreshTokenMeta,
      user.id,
    );
  ...
  async verifyRecentLoginAttempts(email: string): Promise<boolean> {
    const recentAttempts: number =
      await usersRepository.getRecentLoginAttemptsCount(email);
