...
import {articlesQueryRepository, PaginatedArticlesViewModel} from "../repositories/articles/articles-query-repository";
...
const articlePostValidation = [
    ...
    body("synopsis")
        ...
        .escape(),
    body("image").trim().notEmpty().withMessage("Image must be present").escape(),
];
...
export const getArticlesRoutes = () => {
    ...
    router.get(
        "/theme/:theme",
        async (
            req: any, res: Response<PaginatedArticlesViewModel>
        ) => {
            ...
            const foundArticles: PaginatedArticlesViewModel = await foundArticlesPromise
            if (foundArticles) {
                res.status(200).json(foundArticles)
            }
        }
