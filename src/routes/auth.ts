...
export const getAuthRouter = () => {
  ...
  router.post(
    ...
    async (req: RequestWithBody<UserCreateModel>, res: Response) => {
      ...
      if (user) {
        ...
        res
          .status(400)
          .send("Something went wrong, please try again a few minutes later");
      }
    },
  );
    ...
    async (req: RequestWithBody<UserCreateModel>, res: Response) => {
      try {
        ...
        if (!result.success) {
          ...
          const tokens = await usersService.createAuthTokens(req.body.email);
  ...
  router.post(
    ...
    async (req: RequestWithBody<AuthResetPasswordModel>, res: Response) => {
      await usersService.resetPassword(req.body.email);
